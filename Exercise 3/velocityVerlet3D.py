...
def main():
    ...
    with open('nitrogen.dat') as f:
        ...
        initial_wavenumber = 2255.4982206627237
    # Write out initial conditions and interatomic seperation
    ...
    for i in range(numstep):
        # Update particle position and record the interatomic seperation
        p1.update_pos_2nd(dt,force)
        p2.update_pos_2nd(dt, -force)
        r_12 = abs(p2.pos - p1.pos)

        # Update force
        force_new = force_morse(p1, p2, D_e, r_e, alpha)
    ...
    pyplot.title('Velocity Verlet: x-position of interatomic seperationvs vs time')
